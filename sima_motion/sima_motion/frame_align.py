"""2D/3D frame alignment via cross-correlation.

Provides PlaneTranslation2D and VolumeTranslation motion estimation
strategies using pyramid-based cross-correlation alignment.
"""
...
from . import motion
from .align import align_cross_correlation

# Global variables used during parallelized whole frame shifting
lock = 0
namespace = 0
...
def _frame_alignment_base(
    ...
    namespace.pixel_counts = np.zeros(dataset.frame_shape)
...
def _align_frame(inputs):
    """Align a single frame and update the reference image."""
...
def _update_sums_and_counts(
    ...
    """Update pixel sums and counts of the reference image.

    >>> from sima_motion.frame_align import _update_sums_and_counts
    >>> import numpy as np
...
def _resize_array(array, displacement, frame_shape):
    ...
    >>> from sima_motion.frame_align import _resize_array
    >>> import numpy as np
...
class VolumeTranslation(motion.MotionEstimationStrategy):
    ...
    criterion : float, optional
        The number of standard deviations below the mean correlation
        for a displacement to be considered valid.
    """
...
def shifted_corr(reference, image, displacement):
    """Calculate correlation between reference and shifted image."""
...
def pyr_down_3d(image, axes=None):
    ...
    filtered_image = scipy.ndimage.gaussian_filter(image, stdevs)
...
def pyramid_align(reference, target, min_shape=32, max_levels=None,
                  bounds=None):
    """Align target to reference using a multi-resolution pyramid.
    ...
    if max_levels > 0 and np.any(axes_bool):
        ...
        if bounds is None:
            ...
            new_bounds[0] = np.floor(
                bounds[0].astype(float) / (1 + axes_bool))
            new_bounds[1] = np.ceil(
                bounds[1].astype(float) / (1 + axes_bool))
